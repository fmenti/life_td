...
def test_create_mes_teff_st_table():
    ...
    phot = MaskedColumn(
        data=[4269.0712890625, 4000.154541015625, 4235.400390625, 4235.4004],
        name="teff_gspphot",
        mask=[True, False, False, False],
    )  # all but last made up)
    spec = MaskedColumn(
        data=[3060, 4250, 4000, 4108.0],
        name="teff_gspspec",
    ...
    assert (
        gaia["teff_st_ref"][np.where(gaia["teff_st_value"] == 4108.0)]
        == "2022arXiv220800211G GSP-Spec"
    )
    assert (
        gaia["teff_st_qual"][np.where(gaia["teff_st_value"] == 4108.0)] == "D"
    )
