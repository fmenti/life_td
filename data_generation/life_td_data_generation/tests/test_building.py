...
def test_idsjoin_no_mask():
    ...
    cat = Table(
        data=[
            ["* 61 Cyg b|61 Cyg b", "", "GCRV 13273|LTT 16180"],
            [
                "61 Cyg b|markmuster",
                "HD 201091|LSPM J2106+3844N",
                "USNO-B1.0 1287-00443364",
            ],
        ],
        names=["ids1", "ids2"],
        dtype=[object, object],
...
def test_idsjoin_with_mask():
    ...
    a = MaskedColumn(
        data=["* 61 Cyg b|61 Cyg b", "", "GCRV 13273|LTT 16180"],
        name="ids1",
...
def test_idsjoin_duplicates():
    ...
    cat = Table(
        data=[["ID1|ID2", "ID4|ID5"], ["ID2|ID3", "ID4|ID6"]],
        names=["ids1", "ids2"],
        dtype=[object, object],
    )
...
def test_idsjoin_large_table():
    ...
    n_rows = 10_000
    ids1 = [f"ID{i}" for i in range(n_rows)]
    ids2 = [f"ID{i + 1}" for i in range(n_rows)]
...
def test_best_para_membership():
    ...
    wanted_table = Table(
        data=[[1, 3, 1], [2, 4, 5], [100, 20, 999999]],
        names=["child_object_idref", "parent_object_idref", "membership"],
        dtype=[int, int, int],
    )
    ...
    cat = Table(
        {
            ...
            "temp_value": MaskedColumn(
                [100, 200, 300], mask=[False, False, True]
            ),
...
def test_provider_data_merging():
    ...
    provider1_data["sources"] = Table(
        {"ref": ["ref1"], "provider_name": ["provider1"]}
    )
    ...
    provider2_data["sources"] = Table(
        {"ref": ["ref2"], "provider_name": ["provider2"]}
    )
    ...
    empty_prov_dict = {"empty_provider": empty_dict.copy()}
