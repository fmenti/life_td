...
def test_unresolved_binaries():
    ...
    assert (
        ub["unresolved_binaries"][np.where(ub["main_id"] == "star1")] == "False"
    )
    assert (
        ub["unresolved_binaries"][np.where(ub["main_id"] == "system2")]
        == "True"
    )
...
def test_flag_non_main_sequence_stars():
    data = Table(
        (
            ...
            MaskedColumn(
                np.array([0.5, 1.0, 2.0, 999, 1.5, 3.0]),
    ...
    ids = [
        ["no_temp_class", "wrong_temp_class"],
        ["no_lumclass", "wrong_lumclass"],
        ["no_mass"],
    ]
    for id, col in zip(ids, ["ms_temp_class", "ms_lum_class", "mass_flag"]):
        for identifier in id:
            if identifier == "no_mass":
                false = False
            else:
                false = "False"
            assert match_table(catalog, "main_id", identifier)[col] == false
...
def test_flag_trivial_binaries():
    ...
    assert (
        match_table(multiples, "higher_order_multiples", True)["main_id"]
        == "starA"
    )
    assert (
        match_table(multiples, "single_parent", False)["main_id"][0]
        == "multiparent_star"
    )
    assert len(match_table(multiples, "single_parent", False)) == 2
    assert len(match_table(multiples, "trivial_binaries", True)) == 2
...
def test_deal_with_separation():
    ...
    multiples = Table(
        (
            MaskedColumn([1.0, 100.0, np.ma.masked]),
            np.array([True, True, False]),
            np.array([5.0, 20.0, 10.0]),
        ),
    ...
    assert result["sep_phys_value"][0] == 5.0
    assert result["sep_phys_value"][1] == 2000.0
...
def test_assign_critical_separation():
    ...
    multiples = Table(
        (
            np.array(
                ["B_parent", "A_parent", "A_parent", "B_parent", "C_parent"]
            ),
            np.array([True, True, True, True, False]),
            np.array([5.0, 20.0, 10.0, 3.0, 1.0]),
    ...
    row_B_mass_1 = match_table(HZstability, "mass_st_value", 1.0)
...
def test_apply_stability_constraint():
    ...
    HZstability = Table(
        (
            np.array(["A_parent", "A_parent", "B_parent", "B_parent"]),
            np.array(["star1", "star2", "star3", "star4"]),
            np.array([25.0, 20.0, 19.0, 3.0]),
        ),
