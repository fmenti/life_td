...
def query_returns(monkeypatch):
    ...
    t0 = Table({
        'main_id': np.array(['star1','star2','star3','star4'],dtype=object),
        'coo_ra': MaskedColumn([160.,165.,100.,60.]),
        'coo_dec': MaskedColumn([35.,45.,30.,20.]),
        'coo_err_angle': MaskedColumn([90.,17.,np.ma.masked,np.ma.masked]),
        'coo_err_maj': MaskedColumn([0.02,180.,np.ma.masked,np.ma.masked]),
        'coo_err_min': MaskedColumn([0.01,160.,np.ma.masked,np.ma.masked]),
        'oid': [1,2,3,6],
        'coo_ref': np.array(['ref_coo','ref_coo','ref_coo2','ref_coo'],dtype=object),
        'coo_qual': ['A','A','C', 'C'],
        ...
        'plx_value': MaskedColumn([190.,230.,375.,300.]),
        'plx_ref': np.array(['ref_plx','ref_plx','ref_plx2','ref_plx'],dtype=object),
        'plx_qual': MaskedColumn(data=['A','A','C','A'], fill_value='N'),
        'membership': MaskedColumn([np.ma.masked,np.ma.masked,100,np.ma.masked]),
        ...
        'ids': np.array(['star1|id1','star2|id2','star3|id3','star4|id4'],dtype=object),
        'mag_i_value': MaskedColumn([np.ma.masked,np.ma.masked,7.,np.ma.masked]),
        'mag_j_value': MaskedColumn([8.,np.ma.masked,6.,np.ma.masked]),
        'mag_k_value': MaskedColumn([9.,np.ma.masked,8.,np.ma.masked])
    })
    ...
    ident = Table({
        'main_id': np.array(['star1','star1','star3','star3','system1','planet1','star4','star4'],dtype=object),
        'oid': [1,1,3,3,4,5,6,6],
        'id': np.array(['star1','id1','star3','id3','system1','planet1','star4','id4'],dtype=object),
    })
...
def test_create_simbad_helpertable(query_returns):
    ...
    assert set(result_sim_helptab['type']) == set(['sy','sy','sy','pl','st'])
    # objects got correctly added and removed from helptab
    assert len(result_sim_helptab) == 5# stars, systems and planets but removed star2
...
def test_creating_helpertable_stars(query_returns):
    ...
    for null,colname in zip([999999,999999,999999,999999,999999,999999,
                             999999,999999,999999,999999,999999],
                            ['membership','coo_err_angle','coo_err_maj',
                             'coo_err_min','parent_oid','mag_i_value',
                             'mag_j_value','mag_k_value','coo_ra','coo_dec',
                             'plx_value']):
...
def test_create_ident_table(query_returns):
    ...
    expected_ident = Table({
        'main_id': np.array(['star1', 'star1','star3', 'star3','system1', 'planet1','star4','star4'],dtype=object),
        'id_ref': np.array([ref, ref, ref, ref, ref, ref,ref,ref],dtype=object),
        'id': np.array(['id1', 'star1','id3', 'star3','system1', 'planet1','star4','id4'],dtype=object)
    })
...
def expanded_helptab_stars(expected_stars,ref):
    ...
    expected_stars['binary_flag'] = np.array(['True' for j in range(len(expected_stars))],dtype=object)
    expected_stars['binary_flag'][np.where(expected_stars['main_id'] == 'star4')] = 'False'
    ...
    expected_stars['mag_i_ref'] = MaskedColumn(
        data=np.array(['', ref, '',''],
                      dtype=object),
        mask=[True, False, True,True], fill_value='N')
    expected_stars['mag_j_ref'] = MaskedColumn(
        data=np.array([ref, ref, '',''],
                      dtype=object),
        mask=[False, False, True,True], fill_value='N')
    ...
    for col in ['sptype_ref','coo_ref','plx_ref']:
        ...
        expected_stars[col].mask[np.where(expected_stars['main_id'] == 'system1')] = True
...
def test_expanding_helpertable_stars(query_returns):
    ...
    for null,colname in zip([999999,999999,999999,999999,999999,999999,999999,999999,
                            ...
                            ['membership','coo_err_angle','coo_err_maj',
                             ...
                             'coo_ra','coo_dec','plx_value','coo_ref','sptype_ref','plx_ref']):
        result_stars= nullvalues(result_stars, colname, null)
        expected_stars = nullvalues(expected_stars, colname, null)
...
def test_create_object_table(query_returns):
    ...
    expected_object = Table({
        'main_id': np.array(['star1', 'star3', 'system1', 'planet1','star4'],dtype=object),
        'type': np.array(['st', 'sy', 'sy', 'pl','st'],dtype=object),
        'ids': np.array(['star1|id1','star3|id3', 'system1', 'planet1','star4|id4'],dtype=object)
    })
