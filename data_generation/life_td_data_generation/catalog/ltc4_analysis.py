...
def create_updated_ltc3():
    ...
    lost = ltc3[
        np.where(np.invert(np.in1d(ltc3["sim_name"], ltc3_update["sim_name"])))
    ]
    # L  326-61, L  380-78,  L  755-19, LP  399-68 -> maybe were truncated?

    print(
        "To compare the version 3 to version 4 we had to updated the simbad main identifiers of v3."
    )
    print(
        "Of the originally ",
        len(ltc3),
        "objects ",
        len(lost),
        "could not be matched that way.",
    )
    print("After matching on position those could be retrieved again.")
    newltc3 = ltc3
    newltc3["sim_name"][np.where(newltc3["sim_name"] == "L  326-61")] = (
        "L 326-21"
    )
    newltc3["sim_name"][np.where(newltc3["sim_name"] == "L  380-78")] = (
        "L 380-8"
    )
    newltc3["sim_name"][np.where(newltc3["sim_name"] == "L  755-19")] = (
        "LP 755-19"
    )
    newltc3["sim_name"][np.where(newltc3["sim_name"] == "LP  399-68")] = (
        "L 399-68"
    )
    ...
    in_3_but_not_in_4 = newltc3[
        np.where(np.invert(np.in1d(newltc3["main_id"], StarCat4["main_id"])))
    ]
...
def create_hwo():
    ...
    spores_raw = ap.io.ascii.read(
        "data/spores_catalog_v1.2.0.csv"
    )  # ,header_start=0,)
...
def create_hpic():
    ...
    print(HPIC.colnames)
    HPIC = HPIC[
        np.where(HPIC["sy_dist"] != "null")
    ]  # 143 null values in distance
    HPIC = HPIC[
        np.where(HPIC["st_spectype"] != "null")
    ]  # of those about 2600 have no spectral type given
...
def ltc4_comparison(
    create=False, distance_cut_in_pc=30.0, boxplots=True, finalplots=True
):
    """
    ...
    if create:
        ...
        join_34 = ap.table.join(
            common_newltc3, common_StarCat43, keys="main_id"
        )
        ...
        join_hwo4 = ap.table.join(
            common_hwo, common_StarCat4hwo, keys="main_id"
        )
    ...
    if boxplots:
        ...
        df_hwo4 = prepare_dataframe_for_sns(
            join_hwo4, [para_hwo, para_starcat4], ["SPORES", "StarCat4"]
        )
    ...
    if finalplots:
        la.final_plot(
            [
                StarCat4["class_temp", "dist_st_value"],
                newltc3["sim_sptype", "distance"],
            ],
            ["LIFE-StarCat4", "LIFE-StarCat3"],
            path="plots/final_plot_34.png",
        )

        la.final_plot(
            [
                common_StarCat43["class_temp", "dist_st_value"],
                common_newltc3["sim_sptype", "distance"],
            ],
            ["common_LIFE-StarCat4", "common_LIFE-StarCat3"],
            path="plots/final_plot_common_34.png",
        )

        la.final_plot(
            [
                StarCat4["class_temp", "dist_st_value"],
                hwo["st_spectype", "sy_dist"],
            ],
            ["LIFE-StarCat4", "SPORES"],
            path="plots/final_plot_hwo.png",
        )

        la.final_plot(
            [
                common_StarCat4hwo["class_temp", "dist_st_value"],
                common_hwo["st_spectype", "sy_dist"],
            ],
            ["LIFE-StarCat4", "SPORES"],
            path="plots/final_plot_common_hwo.png",
        )
        ...
        la.final_plot(
            ...
            ["SPORES", "LIFE-StarCat4", "HPIC"],
            path="plots/final_plot_hwo_life.png",
        )
    return
