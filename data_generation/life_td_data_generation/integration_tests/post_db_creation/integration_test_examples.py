...
def test_life_db_filter_objects_by_type():
    ...
    adql_query=["""SELECT TOP 10 object_id, main_id FROM life_td.object
                WHERE type='st'"""]
...
def test_life_db_all_children_of_an_object():
    life_db_provider=Table()
    life_db_provider['provider_url']=["http://localhost:8080/tap"]
    adql_query=["""SELECT DISTINCT main_id as Child_main_id, object_id as
                child_object_id
                FROM life_td.h_link
...
def test_life_db_all_parents_of_an_object():
    life_db_provider=Table()
    life_db_provider['provider_url']=["http://localhost:8080/tap"]
    adql_query=["""SELECT DISTINCT main_id as parent_main_id, object_id as 
                parent_object_id
                FROM life_td.h_link
...
def test_life_db_all_specific_measurements_of_an_object():
    ...
    assert type(life_db)==type(Table())
    assert len(life_db) == 1
    assert life_db.colnames == ['object_idref', 'teff_st_value',
                                'teff_st_err', 'teff_st_qual',
                                'teff_st_source_idref', 'id',
                                'id_source_idref']
...
def test_life_db_all_basic_disk_data_from_host_name():
    life_db_provider = Table()
    life_db_provider['provider_url'] = ["http://localhost:8080/tap"]
    adql_query = ["""SELECT DISTINCT main_id disk_main_id, object_id as 
                disk_object_id, db.*
                FROM life_td.h_link
    ...
    assert life_db.colnames == ['disk_main_id', 'disk_object_id',
                                'object_idref', 'rad_value',
                                'rad_err', 'rad_qual', 'rad_rel',
                                'rad_source_idref']

def test_life_db_missing_reliable_measurements():
    life_db_provider = Table()
    life_db_provider['provider_url'] = ["http://localhost:8080/tap"]
    adql_query = ["""SELECT star_ob.main_id as star_name, plx_value, plx_err,
    ...
    assert type(life_db) == type(Table())
    assert life_db.colnames == ['star_name', 'plx_value', 'plx_err',
                                'plx_qual', 'plx_source_idref']

def test_life_db_LIFE_StarCat_candidates():
    life_db_provider = Table()
    life_db_provider['provider_url'] = ["http://localhost:8080/tap"]
    adql_query = ["""SELECT o.main_id, sb.coo_ra, sb.coo_dec, sb.plx_value, 
    ...
    assert type(life_db) == type(Table())
    assert life_db.colnames == ['main_id', 'coo_ra', 'coo_dec',
                                'plx_value', 'dist_st_value',
                                'sptype_string', 'coo_gal_l',
                                'coo_gal_b', 'teff_st_value',
                                'mass_st_value', 'radius_st_value',
                                'binary_flag', 'mag_i_value',
                                'mag_j_value', 'class_lum',
                                'class_temp', 'parent_main_id',
                                'sep_ang_value']
