...
def test_create_exo_helpertable():
    ...
    not_correctly_removed_objects = np.isin(
        exo_helptab["exomercat_name"], removed_objects["exomercat_name"]
    )
    assert (
        exo["provider"]["provider_url"][0]
        == "http://archives.ia2.inaf.it/vo/tap/projects"
    ...
    assert (
        len(
            exo_helptab["exomercat_name"][
                np.where(not_correctly_removed_objects)
            ]
        )
        == 0
    )
...
def bestmass(masstable):
    for i, flag in enumerate(masstable["mass_pl_sini_flag"]):
        ...
        if masstable["bestmass_provenance"][i] == "Mass":
            ...
            if flag == "True":
                masstable["mass_pl_qual"][i] = lower_quality(
                    masstable["mass_pl_qual"][i]
                )
